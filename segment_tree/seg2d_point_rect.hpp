...
// 2D segment tree: point-add update, rectangle-sum query.
// API:
//   SegTree2D st(n, m, v);                   // v is n x m matrix
//   st.update(x, y, val);                    // v[x][y] += val
//   int s = st.query(x1, x2, y1, y2);        // sum over rectangle
// Time:    O(log n * log m) per op
// Memory:  O(n * m)
// Tested:  tests/test_seg2d_point_rect.cpp
