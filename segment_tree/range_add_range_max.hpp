...
// Lazy segment tree: range-add update, range-max query.
// API:
//   LazySegTree st(n, v);
//   st.upd(l, r, val);             // v[i] += val for i in [l, r]
//   int m = st.query(l, r);        // max of v[l..r]
// Time:    O(log n) per op
// Memory:  O(n)
// Tested:  tests/test_range_add_range_max.cpp
